...
public interface ProductRepository extends PagingAndSortingRepository<Product, Long> {
  ...
  List<Product> findAllBySellerUserId(Long sellerUserId);
...
//      "Where a.seller_user_id=:sellerId",nativeQuery = true)
  ...
  @Query(value = "select category_id from product where id=:productId and is_active=1 ", nativeQuery = true)
  Optional<Long> findCategoryidIsActive(Long productId);

  @Query(value = "select * from product where product_name LIKE %:query% and is_active=1 and category_id=:categoryId", nativeQuery = true)
  List<Map<Object, Object>> findAllProductsForCustomer(Long categoryId, Pageable pageable,
      String query);

  @Query(value = "select * from product where is_active=1 and category_id=:categoryId", nativeQuery = true)
  List<Map<Object, Object>> findAllProductsForCustomer(Long categoryId, Pageable pageable);
